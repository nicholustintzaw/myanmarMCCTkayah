...
Table \@ref(tab:fcs1table) presents FCS results by geographic location and by wealth quintiles. There is a clear geographic and wealth gradient with FCS as shown in Figure \@ref(fig:fcs1plot) and Figure \@ref(fig:fcs3plot). Poor FCS is at `r paste(tab[tab$state == "Kayah" & tab$strata == "Hard-to-reach", "poor"], "%", sep = "")` in hard-to-reach areas whilst it is only `r paste(tab[tab$state == "Kayah" & tab$strata == "Urban", "poor"], "%", sep = "")` and `r paste(tab[tab$state == "Kayah" & tab$strata == "Rural", "poor"], "%", sep = "")` in urban and rural areas respectively. In the poor and poorest households, poor FCS is at `r paste(tab[tab$state == "Kayah" & tab$strata == "Poor", "poor"], "%", sep = "")` and `r paste(tab[tab$state == "Kayah" & tab$strata == "Poorest", "poor"], "%", sep = "")` respectively compared to no households with poor FCS in medium, wealthy and wealthiest households.
...
Households in Kayah state are employing very few consumption-based coping strategies (see Table \@ref(tab:ccsi1table)). Use of consumption-based coping strategies is higher in households in hard-to-reach areas (`r round(tab[tab$state == "Kayah" & tab$strata == "Hard-to-reach", "bTotalWeighted"], digits = 1)`) compared to households in urban and rural areas (`r round(tab[tab$state == "Kayah" & tab$strata == "Urban", "bTotalWeighted"], digits = 1)` and `r round(tab[tab$state == "Kayah" & tab$strata == "Rural", "bTotalWeighted"], digits = 1)` respectively) as shown in Figure \@ref(fig:ccsi1plot) and Figure \@ref(fig:ccsi2plot).
...
ggplot(data = tab[tab$sample_component == 1 & tab$geo_state == "Kayah", ], 
  ...
  labs(x = "Food expenditure (1,000 MMK)", y = "HFES (%)") +
  themeSettings
```
...
```{r hfes1plot, echo = FALSE, eval = TRUE, fig.cap = "Household food expenditure share by location type", fig.width = 12, fig.height = 9, fig.align = "center", fig.pos = "H"}
tabLong <- tidyr::pivot_longer(data = tab,
                               cols = vulnerable:low,
                               names_to = "indicators", values_to = "estimate")
...
tabLong$indicators <- factor(tabLong$indicators, 
                             levels = c("low", "medium", "high", "vulnerable"),
                             labels = c("Low", "Medium", "High", "Vulnerable"))
...
ggplot(data = tabLong[tabLong$strata %in% c("Rural", "Urban", "Hard-to-reach") & tabLong$state == "Kayah", ], 
  ...
  #scale_y_continuous(limits = c(0, 100)) +
  scale_fill_manual(values = rev(brewer.pal(n = 4, "RdYlGn"))) +
  labs(x = "", y = "%", fill = "HFES classification") +
  themeSettings
```

```{r hfes2plot, echo = FALSE, eval = TRUE, fig.cap = "Household food expenditure share by wealth quintiles", fig.width = 12, fig.height = 9, fig.align = "center", fig.pos = "H"}
ggplot(data = tabLong[!tabLong$strata %in% c("Rural", "Urban", "Hard-to-reach") & tabLong$state == "Kayah", ], 
       aes(x = strata, y = estimate, fill = indicators)) +
  geom_col(width = 0.5) +
  geom_text(aes(group = indicators, label = paste(round(estimate, digits = 1), "%", sep = "")), 
...
```{r handwashing1plot, echo = FALSE, eval = TRUE, fig.cap = "Handwashing services ladder by location type", fig.width = 12, fig.height = 9, fig.align = "center", fig.pos = "H"}
tabLong <- tidyr::pivot_longer(data = tab,
                               cols = hw14:hw16,
                               names_to = "indicators", values_to = "estimate")
...
```{r handwashing2plot, echo = FALSE, eval = TRUE, fig.cap = "Handwashing services ladder by wealth quintiles", fig.width = 12, fig.height = 9, fig.align = "center", fig.pos = "H"}
ggplot(data = tabLong[!tabLong$strata %in% c("Rural", "Urban", "Hard-to-reach") & tabLong$state == "Kayah", ], 
       aes(x = strata, y = estimate, fill = indicators)) +
  geom_col(width = 0.5) +
  geom_text(aes(group = indicators, label = paste(round(estimate, digits = 1), "%", sep = "")), 
...
```{r epi3plot, echo = FALSE, eval = TRUE, fig.cap = "Immunisation coverage per vaccine by location type", fig.width = 8, fig.height = 17, fig.align = "center", fig.pos = "H"}
tabLong <- tidyr::pivot_longer(data = tab,
...
```{r epi4plot, echo = FALSE, eval = TRUE, fig.cap = "Immunisation coverage per vaccine by wealth quintiles", fig.width = 12, fig.height = 17, fig.align = "center", fig.pos = "H"}
ggplot(data = tabLong[!tabLong$strata %in% c("Rural", "Urban", "Hard-to-reach") & tabLong$state == "Kayah", ], 
       aes(x = strata, y = estimate)) +
  geom_col(width = 0.5, colour = "#70C1B3", fill = "#247BA0") +
  geom_text(aes(group = indicators, label = paste(round(estimate, digits = 1), "%", sep = "")), 
...
#### Prevalence of childhood stunting {#stunting}
