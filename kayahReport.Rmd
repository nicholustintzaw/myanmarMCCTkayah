...
```{r fcs1plot, echo = FALSE, eval = TRUE, fig.cap = "Food consumption score classification by location type", fig.width = 12, fig.height = 9, fig.align = "center", fig.pos = "H"}
...
```{r fcs2plot, echo = FALSE, eval = TRUE, fig.cap = "Food consumption score classification by wealth quintiles", fig.width = 12, fig.height = 9, fig.align = "center", fig.pos = "H"}
ggplot(data = tabLong[!tabLong$strata %in% c("Rural", "Urban", "Hard-to-reach") & tabLong$state == "Kayah", ], 
       aes(x = strata, y = estimate, fill = indicators)) +
  geom_col(width = 0.5) +
  geom_text(aes(group = indicators, label = paste(round(estimate, digits = 1), "%", sep = "")), 
...
ggplot(data = tabLong[tabLong$strata %in% c("Urban", "Rural", "Hard-to-reach") & tabLong$state == "Kayin", ],
  ...
  geom_text(aes(label = round(estimate, digits = 1)), 
            position = position_stack(vjust = 1), size = 5) +
  scale_x_reordered() +
  facet_grid(state ~ strata, scales = "free_x") +
  labs(x = "", y = "%") +
  themeSettings +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
```
...
tab$dob[is.na(tab$dob)] <- lubridate::ymd_hms(tab$enumDate)[is.na(tab$dob)] - tab$age[is.na(tab$dob)]

sample_component <- ifelse(lubridate::ymd_hms(tab$dob) %within% interval(start = lubridate::ymd("2018/09/01"), 
                                                                         end = lubridate::ymd("2018/10/31")), 2, 1)

tab$sample_component <- sample_component
...
ggplot(data = tab[tab$geo_state == "MMR002", ], 
  ...
  geom_vline(xintercept = as.Date("2018-10-01"), stroke = 2, colour = "red") +
  themeSettings
...
ggplot(data = tabLong[tabLong$strata %in% c("Urban", "Rural", "Hard-to-reach") & tabLong$state == "Kayah", ],
  ...
  geom_text(aes(label = round(estimate, digits = 1)), 
            position = position_stack(vjust = 1), size = 5) +
  facet_grid(indicators ~ state, scales = "free_x") +
  labs(x = "", y = "%") +
  themeSettings
```
